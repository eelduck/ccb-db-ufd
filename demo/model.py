...
class Preprocessor:
    def __init__(self, path_to_attr: str, path_to_historical: str):
        self.attr = pd.read_csv(path_to_attr,
                                sep=';',
                                parse_dates=['date_report'],
                                index_col=0) \
            .rename(columns={'Код ДС': 'obj_key'})
        self.historical_data = pd.read_csv(path_to_historical, sep=';', index_col=0,
                                           parse_dates=["ДатаНачалаЗадачи",
                                                        "ДатаОкончанияЗадачи",
                                                        "ДатаначалаБП0",
                                                        "ДатаокончанияБП0",
                                                        "date_report"])
        self.historical_data = self.historical_data[(~((self.historical_data['ПроцентЗавершенияЗадачи']==0) & ((self.historical_data['ДатаНачалаЗадачи'] - self.historical_data['date_report']).dt.days > 8)))] \
    ...
    def add_features(self, df):
        ...
        for col in ["ДатаНачалаЗадачи", "ДатаОкончанияЗадачи", "ДатаначалаБП0",
                    "ДатаокончанияБП0", "date_report"]:
        ...
        data["состояние площадки"] = data["состояние площадки"].fillna(
            "Свободна, передана")
        data["НазваниеЗадачи"] = data["НазваниеЗадачи"].fillna("unknow")
        data["Экспертиза"] = data["Экспертиза"].fillna("unknow")
        data["diff_start"] = (
            ((data["ДатаначалаБП0"] - data["ДатаНачалаЗадачи"]).dt.days).fillna(
                0)).astype(np.int32)
        data["diff_end"] = (
            ((data["ДатаокончанияБП0"] - data["ДатаОкончанияЗадачи"]).dt.days).fillna(
                0)).astype(np.int32)
        ...
        data["is_known_Генпроектировщик"] = data["Генпроектировщик"].isna().astype(
            np.uint8)
        ...
        data["speed1"] = 100 / (
                data["ДатаокончанияБП0"] - data["ДатаначалаБП0"]).dt.days.fillna(0)
        data["speed2"] = 100 / (data["ДатаОкончанияЗадачи"] - data[
            "ДатаНачалаЗадачи"]).dt.days.fillna(0)
        data["spped3"] = data["ПроцентЗавершенияЗадачи"] / (
                data["date_report"] - data["ДатаНачалаЗадачи"]).dt.days.fillna(0)
        data["reserve"] = (data["ДатаОкончанияЗадачи"] - data["date_report"]).dt.days
        data["no_reserve"] = (data["reserve"] < 0).astype(np.int8)

        data["time_task_plan"] = (
            ((data["ДатаокончанияБП0"] - data["ДатаначалаБП0"]).dt.days).fillna(
                0)).astype('int')
        data["time_task_fact"] = (
            ((data["ДатаОкончанияЗадачи"] - data["ДатаНачалаЗадачи"]).dt.days).fillna(
                0)).astype('int')
        ...
        avg_time_task_plan = data.groupby(["Кодзадачи", "НазваниеЗадачи"])[
            "time_task_plan"].mean().reset_index().rename(
            columns={"time_task_plan": "avg_time_task_plan"})
        data = data.merge(
            avg_time_task_plan[["Кодзадачи", "НазваниеЗадачи", "avg_time_task_plan"]],
            how='left', on=["Кодзадачи", "НазваниеЗадачи"])
        ...
        avg_time_task_fact = data.groupby(["Кодзадачи", "НазваниеЗадачи"])[
            "time_task_fact"].mean().reset_index().rename(
            columns={"time_task_fact": "avg_time_task_fact"})
        data = data.merge(avg_time_task_fact, how='left',
                          on=["Кодзадачи", "НазваниеЗадачи"])
        data["diff_avg_time_fact"] = data["time_task_fact"] - data["avg_time_task_fact"]
        # Считаем средние значения факта по контракту и сравниваем каждое значение со средним (в срезе по этопу)
        avg_time_task_fact = data.groupby(["Кодзадачи", "НазваниеЗадачи"])[
            "diff_time_plan_fact"].mean().reset_index().rename(
            columns={"diff_time_plan_fact": "avg_diff_time_plan_fact"})
        data = data.merge(avg_time_task_fact, how='left',
                          on=["Кодзадачи", "НазваниеЗадачи"])
        data["diff_avg_time_plan_fact"] = data["diff_time_plan_fact"] - data[
            "avg_diff_time_plan_fact"]
        ...
        data["time_by_square"] = (data["time_task_fact"] + 1) / (
                data["Площадь"].fillna(0) + 1)
        data["time_by_worker"] = (data["time_task_fact"] + 1) / (
                data["Кол-во рабочих"].fillna(0) + 1)

        # Удаляем временые колонки
        data = data.drop(
            columns=['time_task_plan', 'time_task_fact', 'avg_time_task_fact',
                     'avg_time_task_plan', 'diff_time_plan_fact',
                     'avg_diff_time_plan_fact'])

        full = pd.concat([data, self.historical_data]).sort_values(
            ["obj_key", "Кодзадачи", "НазваниеЗадачи", 'date_report'])
        full["просрок"] = (full.groupby(["obj_key", "Кодзадачи", "НазваниеЗадачи"])[
                               "ДатаОкончанияЗадачи"].transform(
            pd.Series.diff).dt.days).fillna(0)
        full['cumsum_просрок'] = \
            full.groupby(["obj_key", "Кодзадачи", "НазваниеЗадачи"])[
                "просрок"].transform(
                pd.Series.cumsum).fillna(0)
        full = full[
            ["obj_key", "Кодзадачи", "НазваниеЗадачи", "просрок", 'cumsum_просрок']]
        data = data.merge(full, on=["obj_key", "Кодзадачи", "НазваниеЗадачи"],
                          how="left")
        return data
