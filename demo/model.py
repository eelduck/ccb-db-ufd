...
class Preprocessor:
    ...
    def add_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        data["Площадь"] = data["Площадь"].fillna(int(data["Площадь"].mean()))
        data["Кол-во рабочих"] = data["Кол-во рабочих"].fillna(
            int(data["Кол-во рабочих"].mean()))
        data["diff_start"] = (
        ...
        data = pd.get_dummies(data=data, columns=["состояние площадки"], drop_first=True,
                              dtype=np.uint8)
        ...
        data = data.drop(columns=['time_task_plan', 'time_task_fact', 'avg_time_task_fact',
